...
def main():
    ...
    session = tf.Session() 
    writer = tf.summary.FileWriter('./alexnet/train',session.graph)  

    for i in range(80):  
        pascal_classifier.train(input_fn=train_input_fn,steps=500,hooks=[logging_hook])
        pred = list(pascal_classifier.predict(input_fn=eval_input_fn))
        
        if i == 0 or i==10 or i==25 or i==35 or i ==50 or i==60:
            filters = pascal_classifier.get_variable_value("conv2d/kernel")
            filter_montage = filters.transpose(3,0,1,2)
            filter_arr = filter_montage.reshape(12*11,8*11,3)
            img = Image.fromarray(filter_arr,'RGB')
            img.save('conv1_' + str(i) + '.jpg')
