...
def cnn_model_fn(features, labels, mode, num_classes=20):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        # This is where it is different, code is taken from: 
        # https://github.com/ppwwyyxx/tensorpack/blob/master/examples/ResNet/cifar10-preact18-mixup.py
        images = input_layer
        weights = features["w"]
        labels = tf.cast(labels,tf.float32)

        weight = np.random.beta(0.3,0.3,10)
        x_weight = weight.reshape(10, 1, 1, 1)
        y_weight = weight.reshape(10, 1)
...
def main():
    ...
    pascal_classifier = tf.estimator.Estimator(model_fn=partial(cnn_model_fn,num_classes=train_labels.shape[1]),model_dir="./vgg_mixup")
    ...
    session = tf.Session()
    file_writer = tf.summary.FileWriter('./vgg_mixup/train',session.graph)

    for i in range(20):
        pascal_classifier.train(input_fn=train_input_fn,steps=200,hooks=[logging_hook,vgg_init])
        ...
        file_writer.add_summary(summary_map,200*i)
